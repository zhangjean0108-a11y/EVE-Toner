...
function getHpIndigoModelText(product: Product) {
  if (product.id === "1601829082310") {
    return "HP Indigo Q4132D 6000 7000 7900 8000 6K 7K 8K";
  }

  if (product.id === "1601827592845") {
    return "HP Indigo 3000-5900 Series 2";
  }
...
export function getProductCanonicalSlug(product: Product) {
  ...
  if (product.id === "1601827592845") {
    return `hp-indigo-3000-5900-series2-digital-press-ink-${product.id}`;
  }
...
export function getProductSeoTitle(product: Product) {
  if (isHpIndigoInk(product)) {
    const modelText = getHpIndigoModelText(product);
...
export function getProductLongDescription(product: Product) {
  if (isHpIndigoInk(product)) {
    ...
    return `EVE Toner supplies compatible ${modelText} HP Indigo ink with a minimum order of 1 unit in the listed sales pack. Ink and HP Indigo spare parts can be quoted together: send part numbers or photos to confirm matching items and availability. Our professional technical team helps with press-specific compatibility checks, setup guidance and troubleshooting. Confirm your press model, ink code, color and machine configuration before use so we can identify any required preparation. Our focus is print quality comparable to original ink for the matched application; a small initial order lets you evaluate output on your own press and substrate. These are compatible alternatives, not original HP ink. Send your requirements and destination for a quotation.`;
  }
